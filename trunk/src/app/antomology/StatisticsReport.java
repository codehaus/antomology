...
public class StatisticsReport {
    ...
    private static final String[] HEADERS = new String[] {"name", "count", "avg (ms)", "total (ms)", "%" };

    private static final StringFormatter FORMATTER = new StringFormatter();

    public void print(String title, SeriesMap seriesMap) {
        ...
        updateTableWithPercentagesOfTotalTime(table, totalTimes, runningTotalTime);

        System.out.println();
        System.out.println(toString(title, table));
    }

    private void updateTableWithPercentagesOfTotalTime(Table table, long[] totalTimes, long runningTotalTime) {
        for (int i = 0; i < totalTimes.length; i++) {
            long totalTime = totalTimes[i];
            long round = Math.round(100 * (double) totalTime / runningTotalTime);
